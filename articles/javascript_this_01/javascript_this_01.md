...
**注意，由于箭头函数没有自己的this指针，通过 call()，apply()，bind() 方法调用一个箭头函数时，只能传递参数（不能绑定this），他们的第一个参数会被忽略**，看下面的代码  
...
解释一下第5点，因为**this 永远指向最后调用它的那个对象**，那么我们就来找最后调用匿名函数的对象，这就很尴尬了，因为匿名函数没有函数名，所以是没有办法被其他对象调用的，所以说**匿名函数的 this 永远指向 window**  
