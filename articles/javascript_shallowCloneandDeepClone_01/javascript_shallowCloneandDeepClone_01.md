...
参考：
https://juejin.im/post/5f1f5c41f265da22ab2d7db6
...
这是一个递归的深拷贝函数，实现思路就是遍历对象，判断对象的属性值是否还是对象，如果还是那就递归。其实这并不是一个完美的深拷贝函数，还缺少一些边界条件的判断，建议使用 `lodash` 的 `_.cloneDeep`  
...
`a.x = a = {n: 2};` 关键就在这句话，正常的执行顺序应该是 `a = {n:2};a.x = a;`，这里`a.x` 中因为有 `.` 运算符，优先级比 `=` 运算符高，所以会先执行 `a.x = {n:2};`，看下图:  

[![alMSnf.png](https://s1.ax1x.com/2020/07/31/alMSnf.png)](https://imgchr.com/i/alMSnf)  

接下来执行 `a = {n: 2};`，变量 `a` 被重新赋值，指向一个新的地址，继续看下图:  

![alMOVU.png](https://s1.ax1x.com/2020/07/31/alMOVU.png)  
